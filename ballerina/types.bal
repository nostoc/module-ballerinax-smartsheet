...
# Request body schema for copying workspace content to a destination
public type WorkspaceIdCopyBody ContainerDestinationForCopy;

# Response containing proof data with version information
...
# Event triggered when a comment is added to a row, grid, or workspace
...
# Template item data containing report properties and access information
...
# Response object containing a list of webhooks with result metadata
...
# Request body for favorites operations, accepts single favorite or list
...
# Additional details for attachment send events including recipients and location
...
# Individual search result item with object details and context information
...
# Container for home page content data
...
# Request parameters for retrieving filtered events
...
# Detailed information about a webhook including status and statistics
public type WebhookDetails record {
    ...
    # Read-only timestamp when the webhook was created
    Timestamp createdAt?;
...
# Event triggered when user is removed from sheet sharing permissions
public type SheetRemoveShareMember record {
    *Event;
    # The action applied to the specified object
    "REMOVE_SHARE_MEMBER" action?;
    # Details about user or group removed from sheet sharing
    SheetRemoveShareMemberAdditionalDetails additionalDetails?;
    # The Smartsheet resource impacted by the event
    "SHEET" objectType?;
};

# Response containing attachment data with version information
...
# Additional details for dashboard rename events
...
public type GroupRename record {
    *Event;
    # The action applied to the specified object
    "RENAME" action?;
    # Additional details for group rename events including old and new names
    GroupRenameAdditionalDetails additionalDetails?;
    # The Smartsheet resource impacted by the event
    "GROUP" objectType?;
};

# Container for group member list data
public type GroupMembersData record {
    # List of Group Members
    GroupMember[] members?;
};

# Response containing row data and properties
...
# Request body for creating a sheet from scratch or from a template
public type SheetsBody SheetToCreate|SheetToCreateFromTemplate;

# Response data returned when creating a share
...
# Email configuration including recipients, subject, and message
...
# Response returned when creating a proof request
public type ProofRequestCreateResponse record {
    *GenericResult;
    *ProofRequestCreateData;
};

# Response containing a list of workspace shares
...
# Additional details for sheet row sending events
...
# Request body for sheet sharing operations, accepts single or multiple shares
...
# Writeable timestamp value in datetime or numeric format
...
# Report object with scope, source sheets, and summary information
...
# Response object for bulk summary field creation operations
...
# Union of all possible event data types in the system
...
# Automated rule configuration for sheet operations and notifications
...
# Request object for adding a member to a group by email address
public type GroupMemberAdd record {
    # Group member's email address
    string email?;
};

# Event triggered when a new sheet is created in Smartsheet
...
public type ColumnUpdateData record {
    # Column properties for updating sheet column configuration
    UpdateColumn result?;
};
...
# Details for sheet share removal events including user/group information
...
# Container for user profile information response
...
# Summary information for a dashboard in listing views
...
# Container for a list of proof objects
...
# Row object with properties for updating sheet rows
...
# Schema containing a shared secret value for authentication
...
# Details about user added to sheet sharing via group membership
...
# Request body schema for sheet row operations with multiple formats
...
# Contains fields that can be updated on an existing sheet
...
# Represents an action taken on a proof request
...
# Response object for comment creation operations
public type CommentCreateResponse record {
    *GenericResult;
    # Comment object containing text, attachments, and metadata
    Comment result?;
};

# Container for workspace operation results
public type WorkspaceData record {
    # The workspace listing result
    WorkspaceListing result?;
};

# Basic information about a folder item
...
# Response object for comment update operations with version information
...
# Response object for share creation operations
public type ShareCreateResponse record {
    *ItemResult;
    *ShareCreateData;
};

# Event triggered when user or group is removed from sheet sharing
...
# Array of group members with their details and permissions
...
# Event triggered when sheet ownership is transferred to another user
...
# Response containing filtered events from the event stream
...
# Defines a reference to a range of cells in another sheet
...
# Represents a row in a sheet with cells, metadata, and formatting
...
# Configuration settings for publishing reports with read-only access controls
...
# Additional details about removing sharing permissions from workspaces
...
public type UpdateRequestCreate record {
    *Event;
    # The action applied to the specified object
    "CREATE" action?;
    # Additional details for update request creation events
...
# Response object containing summary information and results
...
# Data object for folder creation containing updated user properties
public type FolderCreateData record {
    # Updated User Properties
    AlternateEmail[] data?;
};

# Summary field object with metadata, formatting, and value information
public type SummaryField record {
    ...
    string title?;
    # Data type of the summary field (date, text, checkbox, etc.)
...
# Directive specifying which rows to copy or move and their destination
...
# Complete sheet object containing all properties, data, and metadata
public type Sheet record {
    # Indicates whether multi-select is enabled
    boolean isMultiPicklistEnabled?;
    # Workspace summary information for listing operations
    ...
    Discussion[] discussions?;
    # Source information for objects created from templates or copies
    Source 'source?;
    ...
    boolean readOnly?;
    # Array of row data contained in the sheet
    Row[] rows?;
...
# Response containing a list of proof details with pagination metadata
...
# Data containing alternate email search results
public type AlternateEmailData record {
    # Array of search result items for alternate emails
    SearchResultItem[] result?;
};

# Data containing results of row copy operation
public type RowCopyData record {
    # Array of updated row objects from the copy operation
    UpdateRowsObject[] result?;
};

# Additional details for folder creation events including user and source info
...
# Currency information with symbol and ISO 4217 code
...
# Event triggered when removing workspace sharing permissions from sheet
...
# Response object containing automation rule details
public type AutomationRuleResponse AutomationRule;

# Response containing list of attachments for a specific row
public type RowAttachmentListResponse record {
    *IndexResult;
    *RowAttachmentListData;
};

# Event triggered when a folder is saved as a copy
...
# Object representing a calculated datetime value in RFC 3339 format
...
# Request body for workspace update operations
...
# Event triggered when an access token is authorized
...
# Container for template list data with accessible template items
...
# Response data for proof request creation operation
public type ProofRequestCreateData record {
    # Represents a proof request with status and sender information
    ProofRequest result?;
};

# Event triggered when an admin sends/resends an invitation link to a user
...
# Container for user list data with array of user objects
public type UserListData record {
    # List of User Objects
    User[] data?;
};

# Group object containing group properties and metadata
...
# Event triggered when a user/group is added to sheet sharing or permissions change
...
# Container for sheet publishing operation results
public type SheetPublishData record {
    # Describes the sheet's publish settings
    SheetPublish result?;
};

# Response containing column data and metadata
...
# Event triggered when a user is added to dashboard sharing permissions
...
public type SheetExport record {
    *Event;
    # The action applied to the specified object
    "EXPORT" action?;
    # Additional details for sheet export events including format and user info
    SheetExportAdditionalDetails additionalDetails?;
    # The Smartsheet resource impacted by the event
    "SHEET" objectType?;
};

# Container for workspace sharing list data
...
# Configuration for sending email notifications for multiple rows
...
# Response object containing group information and operation metadata
...
# Response object for home folder creation operations
public type HomeFolderCreateResponse record {
    *GenericResult;
    *HomeFolderCreateData;
};

# Complete webhook configuration including request data and details
...
# Request body for moving a folder to a new container destination
...
# Event triggered when a recurring backup schedule is deleted from workspace
...
# Response data containing the created group information
...
# Update request combining email settings and request details
...
# Cross-sheet reference data container
public type CrossSheetReferenceData record {
    # Can contain dashboards, folders, reports, sheets, or templates
    Folder result?;
};

# Result of bulk operation with failed items information
...
# Response schema for sent update request list operations
public type SentUpdateRequestListResponse record {
    *IndexResult;
    *SentUpdateRequestListData;
};

# Event triggered when a workspace is saved as a new copy
...
# Event triggered when a discussion comment is updated or replied to
...
# Destination container information for move operations
...
# Response object for summary field creation operations
...
# Attributes and properties that define a Smartsheet column
public type ColumnObjectAttributes record {
    ...
    # Column title
    string title?;
    # Column data type defining what kind of values it can contain
    "ABSTRACT_DATETIME"|"CHECKBOX"|"CONTACT_LIST"|"DATE"|"DATETIME"|"DURATION"|"MULTI_CONTACT_LIST"|"MULTI_PICKLIST"|"PICKLIST"|"PREDECESSOR"|"TEXT_NUMBER" 'type?;
    ...
    # Available options for picklist and multi-picklist columns
    string[] options?;
    # Display width of the column in pixels
    decimal width?;
...
# Additional details for workspace sharing events
...
# Container for a list of column objects
public type ColumnListData record {
    # Array of column objects
    GetColumn[] data?;
};

# Container for event stream data with list of events
public type EventStreamData record {
    # List of Events
    EventUnionData[] data?;
};

# Configuration for sending row data via email
...
# Response data containing row attachment cell history
...
public type UserDeclineInviteAdditionalDetails record {
    # Email address of the user responsible for the event
    string emailAddress;
    # Reason code for invitation acceptance failure or decline
...
public type DashboardAddPublish record {
    *Event;
    # The action applied to the specified object
    "ADD_PUBLISH" action?;
    # Additional details about a dashboard publish event
...
# Event triggered when a sheet is deleted
...
# Additional details for sheet rename events including old and new names
...
# Object containing multiple contact values in an array
...
# Container for public template list data with share information
public type PublicTemplateListData record {
    # Array of share objects for public templates
    Share[] result?;
};

# Generic response object with result code and status message
...
# Event triggered when an attachment is deleted from Smartsheet
...
# Cell modification history including timestamp and user information
...
# Response containing group members list with operation result
...
# Event triggered when sharing permissions are removed from a dashboard
...
# Response containing API token information and result status
public type TokenResponse record {
    *Result;
    *TokenData;
};

# Response for workspace folder creation operation
public type WorkspaceFolderCreateResponse record {
    *Result;
    *WorkspaceFolderCreateData;
};

# Event triggered when a cell link is created between sheets
...
# Event triggered when sharing is removed from a workspace dashboard
...
# Response object returned after creating a user
...
# Response object containing discussion information
...
# Response from report publish operation
public type ReportPublishResponse record {
    *ItemResult;
    *ReportPublishData;
};

# Data containing cross-sheet reference information
public type CrossSheetReferenceListData record {
    # Sheet object containing basic sheet information and properties
...
# Summary data containing result information for summary field operations
public type SummaryData record {
    # Summary field with image attachment capabilities
    SummaryFieldAddImage result?;
};

# Additional details for report sharing events including user and access info
...
# Response data for workspace share creation
...
# Event triggered when a report is exported or printed
...
# Response returned when creating an update request
public type UpdateRequestCreateResponse record {
    *Result;
    *UpdateRequestCreateData;
};

# User information for a group member
...
# Data container for created discussion information
...
# Container for workspace folder list data with webhook information
public type WorkspaceFolderListData record {
    # list of Webhooks
    Webhook[] data?;
};

# Individual sight item in list responses with timestamps
public type SightListItem SightResult;

# Response object for row move operations combining result and data
...
# Additional details for folder export events including user and format info
...
# Data containing the results of a row move operation
public type RowMoveData record {
    # Array of row objects resulting from the move operation
    AddRowsObject[] result?;
};

# Additional details about an attachment creation event
...
# Event triggered when publish option is disabled for a dashboard
...
# Request body for creating a sheet in a folder
...
# Event triggered when an attachment is sent via email to users or groups
...
# Container for dashboard list data
...
# Response object for automation rule update operations
public type AutomationRuleUpdateResponse record {
    *Result;
    *FavoriteData;
};

# Container for webhook list response data
...
# Container for workspace template data with an array of templates
...
# Request object for updating the completion status of a proof
public type UpdateProofStatusRequest record {
    # Boolean flag indicating whether the proof is marked as completed
    boolean isCompleted?;
};

# Configuration object defining delivery schedule timing and frequency
public type Schedule record {
    ...
    # End date and time for the delivery schedule with timezone information
    TimestampWriteable endAt?;
    # Timestamp of when the next scheduled request will be sent
    Timestamp nextSendAt?;
    # Start date and time for the delivery schedule with timezone information
    TimestampWriteable startAt?;
};

# Response object containing a list of cross-sheet references with result metadata
...
# Response object for workspace share creation operations
public type WorkspaceShareCreateResponse record {
    *Result;
    *WorkspaceShareCreateData;
};

# Container for cell history operation results
...
# Container for webhook event data and results
public type WebhookData record {
    # Webhook event result data (sheet creation events)
    SheetCreated|SheetCreatedFromTemplate result?;
};

# Container for favorites operation results
public type FavoritesResultData record {
    # Single favorite or array of favorite items
...
# Represents an alternate email address associated with a user account
...
# OAuth token object containing access credentials for API authentication
...
# Response object returned after creating rows in a sheet
public type RowCreateResponse record {
    *Result;
    *RowCreateData;
};

# Container object for share operation result data
public type ShareData record {
    # Dashboard object containing widgets, layout, and workspace information
    Sight result?;
};
...
# Container for folder attachment data array
...
# Context details for report creation events including source information
...
# Event triggered when a new report is created
...
# Event triggered when transferring group ownership between users
...
# Details for report share member removal including user and group context
...
# Event triggered when report ownership is transferred to another user
...
# Response object for deleting workspace share permissions
...
# Column specification for adding new columns to a sheet
...
# Details and metadata for an update request
...
# Array of share objects for report sharing permissions
public type ReportShareArray Share[];

# Container for shared secret result data
public type SharedSecretData record {
    # Folder object containing shared secret information
    Folder result?;
};
...
# Font family configuration with name and platform traits
...
# Response data containing updated sharing information
...
# Represents a row object returned when adding rows to a sheet
...
# Response object containing a list of users with pagination metadata
...
public type DashboardMove record {
    *Event;
    # The action applied to the specified object
    "MOVE" action?;
    # Additional details for dashboard move events
    DashboardMoveAdditionalDetails additionalDetails?;
    # The Smartsheet resource impacted by the event
    "DASHBOARD" objectType?;
};

# Generic result object for operations that may update sheet data
...
# Container for folder contents data
...
# Event triggered when a user is added to a group that has access to a report
...
# Request body schema for workspace creation operations
public type WorkspacesBody record {
    # Workspace name
    string name?;
};

# Required fields for creating a webhook
...
# Represents a cell within a sheet containing data and formatting
public type Cell record {
    ...
    # Array of cell links that this cell references
...
# User profile response including profile image data
...
# Event triggered when a user is removed from report sharing permissions
...
# Container for a list of user groups with associated data
public type GroupData record {
    # List of Groups
    Group[] data?;
};

# Response containing a list of sheet rows with operation results
...
# Defines sort criteria and priority order for sorting operations
...
# Container for row operation results with numeric identifiers
public type RowListData record {
    # Array of numeric row identifiers returned from operation
    decimal[] result?;
};
...
# Event triggered when admin removes user from all groups and shares
...
# Response containing image URL mapping information
public type ImageUrlMapResponse ImageUrlMap;

# Contains alternate email results from folder copy operations
...
# Additional details about row copy operations between sheets
...
# Container for search results data
...
public type UserData record {
    # Array of share objects associated with the user
    Share[] result?;
};
...
# Response containing either a Sheet or SheetVersion favorite object
...
public type FormCreate record {
    *Event;
    # The action applied to the specified object
    "CREATE" action?;
    # Additional details for form creation events
...
# Column object with properties like title, type, and validation settings
...
# Event triggered when sharing permissions are added or changed for a report
...
# Event triggered when workspace ownership is transferred to another user
...
# Event triggered when an admin downloads the login history report
...
# Response containing discussion with attachment list and version info
...
# Event triggered when a workspace is exported by a user
...
# Request to add an alternate email address to user account
public type AddAlternateEmail record {
    # User's alternate email address
    string email?;
};

# Event for downloading published items report by account admin
...
# Event triggered when a sheet backup is requested
...
# Container for discussion list result data
public type DiscussionListData record {
    # Defines a reference to a range of cells in another sheet
    CrossSheetReference result?;
};

# Event triggered when a dashboard is created via UI or API
...
# Response containing cell history data and operation result
...
# Request body for sheet row operations (single row or array of rows)
...
# Task dependency relationship with lag time and critical path information
...
# Container for search results and total count information
...
# Inline body item for application/json image URL requests
...
# Request body for group member operations - single member or member list
...
# Response containing proof attachment list with pagination metadata
...
# Represents a sent update request with recipient, status, and content details
...
# Container for newly created row result data
public type RowCreateData record {
    # Represents a row in a sheet with cells, metadata, and formatting
    Row result?;
};

# Row object containing cells, metadata, and sheet association details
...
# Response containing result status and deleted summary field data
public type SummaryFieldDeleteResponse record {
    *Result;
    *SummaryFieldDeleteData;
};

# Image URL information with dimensions and temporary access link
...
# Response containing paginated list of dashboards
...
# Event triggered when a user accepts an organization invitation
...
# Response data containing report publish information
public type ReportPublishData record {
    # Configuration settings for publishing reports with read-only access controls
    ReportPublish result?;
};

# Event triggered when admin downloads user list report
...
# Event triggered when sharing permissions are added or changed for workspace
...
# Event triggered when organization account is renamed
...
# Response containing a list of update requests with pagination
...
# Response containing home folder contents with pagination
...
# Response for discussion creation with result data
public type DiscussionCreateResponse record {
    *IndexResult;
    *DiscussionCreateData;
};

# Container for proof list result data
...
# Container for share token result data
public type TokenData record {
    # Single Share object or array of Share objects
    Share|Share[] result?;
};
...
# Response containing list of templates with pagination info
...
# Response data containing cross-sheet references
...
# Response containing paginated folder list data
...
# Contains fields that can be updated on an existing webhook
...
# Version information for a sheet
...
# Cell data and metadata within a row context
...
# Event triggered when an attachment is loaded, viewed, or downloaded
...
# Event data for report deletion operations
...
# Response containing a list of contacts with pagination
...
# Sheet metadata including timestamps, access level, and identifiers
...
# Filtered event data with access restrictions
...
# Event triggered when sharing is removed from a workspace
...
# Event triggered when a sheet is saved as a new copy
...
# Container for dashboard name information
...
# Response object containing a list of discussions with result metadata
...
# Event triggered when a user is added to a workspace share
...
# Event triggered when a recurring backup is scheduled for a workspace
...
# Event triggered when a discussion thread is sent via email to users
...
# Pagination metadata for result sets with page and count information
...
# Event triggered when a user is added to a sheet's sharing list
...
# Request body for webhook creation and configuration
...
# Favorited item in Smartsheet workspace
...
# Response containing a list of folders within a workspace
public type WorkspaceFolderListResponse record {
    *IndexResult;
    *WorkspaceFolderListData;
};

# Response containing a list of proof discussions
...
# Response from column update operation
...
# Container for contact list data with array of contact objects
...
# Response containing public templates with pagination metadata
